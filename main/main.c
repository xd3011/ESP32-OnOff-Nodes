...
#define TAG "EXAMPLE"
#define BLE_MESH_DEVICE_NAME "Light ESP32 BLE Mesh"
...
static void prov_complete(uint16_t net_idx, uint16_t addr, uint8_t flags,
    ...
    board_led_operation(LED_PIN, LED_OFF);
}
...
static void example_handle_gen_onoff_msg(
    esp_ble_mesh_model_t *model, esp_ble_mesh_msg_ctx_t *ctx,
    esp_ble_mesh_server_recv_gen_onoff_set_t *set) {
    esp_ble_mesh_gen_onoff_srv_t *srv =
        (esp_ble_mesh_gen_onoff_srv_t *)model->user_data;
    switch (ctx->recv_op) {
        case ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_GET:
            esp_ble_mesh_server_model_send_msg(
                model, ctx, ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_STATUS,
        ...
        case ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_SET_UNACK:
            ...
            esp_ble_mesh_model_publish(
                model, ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_STATUS,
...
void server_send_to_client(uint8_t onoff) {
    // esp_ble_mesh_gen_onoff_state_t state;
    // state.onoff = onoff;
    // esp_ble_mesh_model_publish(onoff_server_0.model,
    //                            ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_STATUS,
    //                            sizeof(state.onoff), &state.onoff, ROLE_NODE);
    ESP_LOGI(TAG, "Send Status");
    esp_ble_mesh_msg_ctx_t _ctx = {0};
    esp_ble_mesh_gen_onoff_state_t _state = {
        .onoff = 0,
        .target_onoff = 1,
    };
    _ctx.send_ttl = 16;
    ...
    ESP_LOGI(TAG, "net_idx: %d, app_idx: %d", _ctx.net_idx, _ctx.app_idx);
    ESP_LOGI(TAG, "addr: 0x%04x, recv_dst: 0x%04x, send_ttl: %d", _ctx.addr,
             _ctx.recv_cred, _ctx.send_ttl);

    esp_err_t err = esp_ble_mesh_server_model_send_msg(
        onoff_server_0.model, &_ctx, ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_STATUS,
        sizeof(_state.target_onoff), &_state.target_onoff);
    if (err != ESP_OK) {
        ESP_LOGE(TAG, "err 0x%04x", err);
    }
}
...
static void example_ble_mesh_provisioning_cb(
    ...
    switch (event) {
        ...
        case ESP_BLE_MESH_NODE_PROV_RESET_EVT:
            ...
            if (err != ESP_OK) {
                ESP_LOGE(TAG, "Failed to reset node, err_code %d", err);
            }
            esp_restart();
            break;
    ...
    switch (event) {
        ...
        case ESP_BLE_MESH_GENERIC_SERVER_RECV_SET_MSG_EVT:
            ...
            if (param->ctx.recv_op == ESP_BLE_MESH_MODEL_OP_GEN_ONOFF_SET ||
                ...
                example_handle_gen_onoff_msg(param->model, &param->ctx,
                                             &param->value.set.onoff);
            }
            break;
        default:
    ...
    if (event == ESP_BLE_MESH_CFG_SERVER_STATE_CHANGE_EVT) {
        switch (param->ctx.recv_op) {
            ...
            case ESP_BLE_MESH_MODEL_OP_MODEL_APP_BIND:
                ...
                ESP_LOGI(TAG,
                         "elem_addr 0x%04x, app_idx 0x%04x, cid 0x%04x, mod_id "
                         "0x%04x",
            ...
            case ESP_BLE_MESH_MODEL_OP_MODEL_SUB_ADD:
                ...
                ESP_LOGI(TAG,
                         "elem_addr 0x%04x, sub_addr 0x%04x, cid 0x%04x, "
                         "mod_id 0x%04x",
...
static esp_err_t ble_mesh_init(void) {
    ...
    board_led_operation(LED_PIN, LED_ON);
